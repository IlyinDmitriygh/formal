...
//      if (finish_vertexes.contains(start)) {
//        return true;
//      }
//      return false;
//    }
...
//  priorities['*'] = 1;
//  priorities['+'] = 3;
//  string s = "(a+b)*$(e)$(e$e$a+b*$e)*";
//  std::cout << create_postfix_write(s) << '\n';
//  auto my_s = create_postfix_write(s);
//  auto v = create_machine_from_postfix(my_s);
//  vector<vector<pair<int, char>>> graph;
//  map<Node*, int> finish_vert;
//  auto counts = create_graph(v, graph, finish_vert);
//  std::cout << counts << '\n';
//  int co = 0;
//  /*for (auto v : graph) {
//    std::cout << co << " : ";
//    for (auto u: v) {
//      std::cout << u.first << " " << u.second << "  next  ";
//    }
//    std::cout << '\n';
//    co += 1;
//  }*/
//  std::cout<<'\n';
//  auto type_vertexes = compress_components(graph, counts, finish_vert);
...
//  // здесь мы получили новый граф с пожатыми компонентами связности и новые завершающие и стартовые состояния.
//
//  // теперь надо избавится от e - переходов, у меня нет повторяющихся ребер, и есть мапа с eps переходами
//  /*std::cout<<'\n';
//  for (auto v : graph) {
//    std::cout << co << " : ";
//    for (auto u: v) {
...
//  }*/
//
//  for (auto v : type_vertexes.first.second) {
//    std::cout << v.first << " ";
//  }
//  std::cout << " " << type_vertexes.second;
//  // Научились удалять eps переходы и избавлятся от вершин до которых не дотягивается BFS
//
//  // Алгоритм Томпсона
//  vector<char> sigma = {'e', 'a', 'b'};
//
...
//  auto new_graph_as_map = result.second;
//  auto finish_points= result.first.second;
//  // Проверим наш алгос когда дойдем до минимализации, нужна еще мусорка
//  addTrash(sigma, new_graph_as_map, alreadyInSet);
//
//  // Начинаем минимизацмю
//  std::set<int> start = {type_vertexes.second};
//  auto all = minimization(new_graph_as_map, sigma, alreadyInSet, finish_points, start);
//  auto mpdka = all.first;
//
...
//    std::cout << '\n';
//  }
//  std::cout << all.second.second;
//
//  auto ans = getRegularExpression(mpdka, all.second.first, all.second.second);
//  std::cout << ans;
//
//
//}
